...
function getAllRuns(gameID, callback, offset, categorie){
	...
	if(offset<9999){
		...
		makeAPIrequest("https://www.speedrun.com/api/v1/runs?game="+gameID+"&max="+max+"&embed=players&status=verified&category="+categorie+"&direction=desc&offset="+offset, (x) => ifDone(x));
		offset += max;
	    } 
	}
	else {
